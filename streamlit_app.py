...
import numpy as np
from PIL import Image, ImageFilter  # <--- Added ImageFilter for cleaning
...
def run_image_analysis(image):
    # ACCURACY FIX: Resize input to 1024x1024 (Model's native resolution)
    # This makes the model more accurate AND prevents RAM crashes with large images.
...
def get_prediction_image(mask):
    ...
    # ACCURACY BOOST: Apply a Median Filter to remove random noise dots.
    # This uses PIL (already installed) so it is very fast and stable.
...
def generate_reef_report(mask, filename):
    ...
    if total_coral == 0: total_coral = 1
    
    lcc_val = (total_coral / total_px) * 100
    sev_val = (bleached_px / total_coral * 100)
